...
[AddComponentMenu("NGUI/Custom/Sprite Custom Animation")]
public class UICustomSpriteAnimation : MonoBehaviour
{
	/// <summary>
	...
	[SerializeField] protected bool mLoop = false;
		...
		if (mActive && mSpriteNames.Count > 1 && Application.isPlaying && mFPS > 0)
		{
			...
			while (rate < mDelta)
			{
				...
				if (++frameIndex >= mSpriteNames.Count)
				{
					...
					mActive = mLoop;
					if(!mActive){
						mSprite.spriteName = mSpriteNames[mSpriteNames.Count-1];
						enabled = false;
						Destroy(gameObject);
					}
				}
